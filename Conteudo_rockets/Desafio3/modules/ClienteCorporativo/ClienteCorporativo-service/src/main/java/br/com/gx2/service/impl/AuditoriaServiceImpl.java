...
import br.com.gx2.service.base.AuditoriaServiceBaseImpl;
...
/**
 * A implementação do serviço remoto de auditoria.
 * Esta classe fornece métodos para adicionar, deletar e recuperar entradas de auditoria.
 * O acesso a esta camada é seguro, pois pode ser exposto para clientes externos.
 */
@Component(
		property = {
				"json.web.service.context.name=ccorp",
				"json.web.service.context.path=Auditoria"
		},
		service = AopService.class,
		scope = ServiceScope.PROTOTYPE
)
public class AuditoriaServiceImpl extends AuditoriaServiceBaseImpl {

	private static final Log _log = LogFactoryUtil.getLog(AuditoriaServiceImpl.class);
	...
	public Auditoria addAuditoria(Auditoria auditoria, ServiceContext serviceContext) throws PortalException {
		...
		try {
			Auditoria addedAuditoria = auditoriaLocalService.addAuditoria(auditoria, serviceContext);
	...
	public Auditoria deleteAuditoria(long auditoriaId) throws PortalException {
		...
		Auditoria auditoria = auditoriaLocalService.deleteAuditoria(auditoriaId);
	...
	public List<Auditoria> getAuditorias(int start, int end) {
		...
		return auditoriaLocalService.getAuditorias(start, end);
	}
	...
	public long countAuditorias() {
		long count = auditoriaLocalService.countAuditorias();
	...
	public Auditoria getAuditoriaById(long auditoriaId) throws PortalException {
		...
		return auditoriaLocalService.getAuditoriaById(auditoriaId);
	}
